...
public class OutcomesPanel extends JPanel {
	...
	private void configurePanel(ResultSet rs) {
		...
		String[] columnNames = {"Entry #", "Date", "Outcome", "Wager", "Old Balance", "New Balance"};
		...
		try {
			...
			for (int i = 0; rs.next(); i++) {
				...
				Object[] arr = {new Integer(i+1), date, outcome, wager, old_bal, new_bal};
				data[i] = arr;
			}
		} catch (SQLException e) {
			System.err.println(e.getMessage());
	...
	public ResultSet getData(String id) {
		...
		ResultSet rs = null;
		try {
			String query = "SELECT date, outcome, wager, old_bal, new_bal FROM outcomes WHERE user_id = " + id;
